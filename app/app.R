...
ui <- page_navbar(
  ...
  nav_panel("Quantitative plots",
    navset_tab(
      ...
      nav_panel("Page 2",
        quant_page(
      analysis_card("exprhm", "Expression heatmap", tagList(numericInput("hm_top", "Top variable proteins", 100, min = 5), selectInput("hm_row_cluster", "Row clustering", choices = c("Hierarchical" = "hclust", "K-means" = "kmeans", "None" = "none")), conditionalPanel("input.hm_row_cluster == 'kmeans'", numericInput("hm_row_k", "Row K-means k", 4, min = 2)), selectInput("hm_col_cluster", "Column clustering", choices = c("Hierarchical" = "hclust", "K-means" = "kmeans", "None" = "none")), conditionalPanel("input.hm_col_cluster == 'kmeans'", numericInput("hm_col_k", "Column K-means k", 4, min = 2)), heatmap_size_inputs("exprhm"), export_input("exprhm"), actionButton("run_exprhm", "Generate heatmap", class = "btn-primary"))),
      ...
      analysis_card("feature_hm", "Feature-protein heatmap", tagList(selectInput("feature_hm_source", "Selected feature source", choices = feature_source_choices, selected = "rf"), numericInput("feature_top_hm", "Use top N selected features", 50, min = 5), selectInput("feature_hm_row_cluster", "Row clustering", choices = c("Hierarchical" = "hclust", "K-means" = "kmeans", "None" = "none")), conditionalPanel("input.feature_hm_row_cluster == 'kmeans'", numericInput("feature_hm_row_k", "Row K-means k", 4, min = 2)), selectInput("feature_hm_col_cluster", "Column clustering", choices = c("Hierarchical" = "hclust", "K-means" = "kmeans", "None" = "none")), conditionalPanel("input.feature_hm_col_cluster == 'kmeans'", numericInput("feature_hm_col_k", "Column K-means k", 4, min = 2)), heatmap_size_inputs("feature_hm"), export_input("feature_hm"), actionButton("run_feature_hm", "Generate feature heatmap", class = "btn-primary")))
        )
      ),
      nav_panel("Page 3",
        quant_page(
...
server <- function(input, output, session) {
  ...
  observeEvent(input$reset_exprhm, { updateNumericInput(session, "hm_top", value = 100); updateSelectInput(session, "hm_row_cluster", selected = "hclust"); updateNumericInput(session, "hm_row_k", value = 4); updateSelectInput(session, "hm_col_cluster", selected = "hclust"); updateNumericInput(session, "hm_col_k", value = 4); reset_common("exprhm", palette = FALSE, width_pt = default_expr_heatmap_width_pt, height_pt = default_expr_heatmap_height_pt) })
  ...
  observeEvent(input$reset_feature_hm, { updateSelectInput(session, "feature_hm_source", selected = "rf"); updateNumericInput(session, "feature_top_hm", value = 50); updateSelectInput(session, "feature_hm_row_cluster", selected = "hclust"); updateNumericInput(session, "feature_hm_row_k", value = 4); updateSelectInput(session, "feature_hm_col_cluster", selected = "hclust"); updateNumericInput(session, "feature_hm_col_k", value = 4); reset_common("feature_hm", palette = FALSE, width_pt = default_expr_heatmap_width_pt, height_pt = default_expr_heatmap_height_pt) })
  ...
  observeEvent(input$run_exprhm, { run_analysis("exprhm", "Expression heatmap", { req(rv$data); d <- analysis_dir("exprhm"); wh <- pts("exprhm"); pdf <- file.path(d, "expression_heatmap.pdf"); csv <- file.path(d, "expression_heatmap_values.csv"); plot_expression_heatmap(data_for_analysis()$quantity, group_info(), pdf, csv, input$hm_top, input$hm_row_cluster, input$hm_col_cluster, input$hm_row_k, input$hm_col_k, max(wh[1], 3), max(wh[2], 3)); finish("exprhm", pdf, list.files(d, pattern = "\\.csv$", full.names = TRUE), input$exprhm_export_csv, standard_mode_note()) }) })
